...
export async function readDiaRange(fechaInicio: string, fechaFinal: string): Promise<DiaRead[]> {
...
export async function updateDia(fecha: string, dia: DiaUpdate): Promise<DiaRead> {
  const res = await fetch(`${URL}/${fecha}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(dia)
