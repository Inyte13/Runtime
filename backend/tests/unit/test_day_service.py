...
def test_build_day_calendar():
  ...
  activity_b1 = category_b.activities[1]
  ...
  assert activity_c1.descriptions == []

  assert activity_c2.id == activity_c2_id
  assert activity_c2.duration == GRANULARITY_HOURS
  assert activity_c2.descriptions == ['Descripción 4']

  assert category_b.id == category_b_id
  assert category_b.duration == GRANULARITY_HOURS * 3

  assert activity_b2.id == activity_b2_id
  assert activity_b2.duration == GRANULARITY_HOURS * 2
  assert activity_b2.descriptions == ['Descripción 2']

  assert activity_b1.id == activity_b1_id
  assert activity_b1.duration == GRANULARITY_HOURS
  assert activity_b1.descriptions == ['Descripción 3']
